...
describe('Lesson 169: 完成と振り返り - ドキュメントテスト', () => {
  let dom;
  let window;
  let document;
  ...
  describe('ドキュメントファイルの存在確認', () => {
    ...
    it('LEARNING_LOG.mdが存在する', () => {
      const learningLogPath = path.resolve(__dirname, 'LEARNING_LOG.md');
      expect(fs.existsSync(learningLogPath)).toBe(true);
    });
  });

  describe('README.mdの内容確認', () => {
    it('README.mdにプロジェクト名が記載されている', () => {
      const readmePath = path.resolve(__dirname, 'README.md');
      const content = fs.readFileSync(readmePath, 'utf-8');
      expect(content).toContain('予算管理アプリ');
    });

    it('README.mdに機能一覧が記載されている', () => {
      const readmePath = path.resolve(__dirname, 'README.md');
      const content = fs.readFileSync(readmePath, 'utf-8');
      expect(content).toContain('機能');
    });

    it('README.mdに使用技術が記載されている', () => {
      const readmePath = path.resolve(__dirname, 'README.md');
      const content = fs.readFileSync(readmePath, 'utf-8');
      expect(content).toContain('使用技術');
    });

    it('README.mdに使い方が記載されている', () => {
      const readmePath = path.resolve(__dirname, 'README.md');
      const content = fs.readFileSync(readmePath, 'utf-8');
  ...
  describe('USAGE.mdの内容確認', () => {
    it('USAGE.mdに基本的な使い方が記載されている', () => {
      const usagePath = path.resolve(__dirname, 'USAGE.md');
      const content = fs.readFileSync(usagePath, 'utf-8');
      expect(content).toContain('基本的な使い方');
    });

    it('USAGE.mdにレポート機能の説明がある', () => {
      const usagePath = path.resolve(__dirname, 'USAGE.md');
      const content = fs.readFileSync(usagePath, 'utf-8');
      expect(content).toContain('レポート機能');
    });

    it('USAGE.mdにトラブルシューティングがある', () => {
      const usagePath = path.resolve(__dirname, 'USAGE.md');
      const content = fs.readFileSync(usagePath, 'utf-8');
  ...
  describe('LEARNING_LOG.mdの内容確認', () => {
    it('LEARNING_LOG.mdにプロジェクト情報が記載されている', () => {
      const learningLogPath = path.resolve(__dirname, 'LEARNING_LOG.md');
      const content = fs.readFileSync(learningLogPath, 'utf-8');
      expect(content).toContain('プロジェクト情報');
    });

    it('LEARNING_LOG.mdに習得したスキルが記載されている', () => {
      const learningLogPath = path.resolve(__dirname, 'LEARNING_LOG.md');
      const content = fs.readFileSync(learningLogPath, 'utf-8');
      expect(content).toContain('習得した');
    });

    it('LEARNING_LOG.mdに難しかった点が記載されている', () => {
      const learningLogPath = path.resolve(__dirname, 'LEARNING_LOG.md');
      const content = fs.readFileSync(learningLogPath, 'utf-8');
      expect(content).toContain('難しかった');
    });

    it('LEARNING_LOG.mdに感想が記載されている', () => {
      const learningLogPath = path.resolve(__dirname, 'LEARNING_LOG.md');
      const content = fs.readFileSync(learningLogPath, 'utf-8');
      expect(content).toContain('感想');
    });
  });
});
