...
function parseCSV(csvText) {
  ...
  for (var i = 1; i < lines.length; i++) {
    ...
    var item = {
      ...
      category: columns[2].trim().replace(/^"|"$/g, ''),
...
function importJSON(file) {
  ...
  reader.onload = function(event) {
    try {
      ...
      showToast('JSONファイルを読み込みました: ' + validData.length + '件', 'success');
...
function importCSV(file) {
  ...
  reader.onload = function(event) {
    try {
      ...
      mergeTransactions(validData);
      showToast('CSVファイルを読み込みました: ' + validData.length + '件', 'success');
...
if (typeof window !== 'undefined') {
  ...
  window.mergeTransactions = mergeTransactions;
