...
describe('Lesson 141: オブジェクトのコピー (Solution)', () => {
  let dom;
  let document;
  let window;
  ...
  describe('オブジェクトの定義', () => {
    it('userオブジェクトが定義されている', () => {
      ...
      expect(user.username).toBe('taro123');
    ...
    it('userRefが定義されている', () => {
      const userRef = window.eval('userRef');
      expect(userRef).toBeDefined();
    });

    it('userAssignが定義されている', () => {
      const userAssign = window.eval('userAssign');
      expect(userAssign).toBeDefined();
    });

    it('userSpreadが定義されている', () => {
      const userSpread = window.eval('userSpread');
      expect(userSpread).toBeDefined();
    });

    it('premiumUserが定義されている', () => {
      const premiumUser = window.eval('premiumUser');
  ...
  describe('2. Object.assign()', () => {
    ...
    it('元のuserとuserAssignが独立している', () => {
      const user = window.eval('user');
      const userAssign = window.eval('userAssign');
      expect(user.email).toBe('taro@example.com');
      expect(userAssign.email).toBe('taro@example.com');
    });
  });
  ...
  describe('3. スプレッド構文', () => {
    ...
    it('元のuserとuserSpreadが独立している', () => {
      const user = window.eval('user');
      const userSpread = window.eval('userSpread');
      expect(user.age).toBe(28);
      expect(userSpread.age).toBe(28);
    });
  });
  ...
  describe('4. プレミアムアップグレード', () => {
    ...
    it('プレミアムステータスの説明が含まれている', () => {
      const upgradeResult = document.getElementById('upgradeResult');
      const content = upgradeResult.innerHTML.toLowerCase();
      expect(content).toContain('premium');
    });

    it('premiumUserのpremiumがtrueである', () => {
      const premiumUser = window.eval('premiumUser');
      expect(premiumUser.premium).toBe(true);
    });

    it('元のuserのpremiumはfalseのまま', () => {
      const user = window.eval('user');
      expect(user.premium).toBe(false);
    });
  });
